        ...
        try:
            with open(JOB_INDEX_FILE, "r", encoding="utf-8") as f:
                ...
                result: dict[tuple[str, str], dict[str, Any]] = {
                    (a, b): v for k, v in data.items() for a, b in [k.split("||")]
                }
                return result
        except Exception as e:
            logger.warning(
                f"Failed to load job index: {e}. Scanning workspace directories..."
            )
    ...
    for user_dir in os.listdir(API_WORKSPACES_DIR):
        ...
        for paper_dir in os.listdir(user_path):
            ...
            created_at = datetime.fromtimestamp(
                os.path.getctime(paper_path)
            ).isoformat()
            updated_at = datetime.fromtimestamp(
                os.path.getmtime(paper_path)
            ).isoformat()
...
class ArxivGenerateRequest(BaseModel):
    ...
    arxiv_id: str = Field(
        ...,
        description="arXiv paper ID (e.g., 2505.18102)",
        json_schema_extra={"example": "2505.18102"},
    )
    user_id: str = Field(..., description="User ID for multi-user isolation")
    api_key: Optional[str] = Field(
        default=None,
        description="OpenAI/LLM API key (optional, uses server default if not provided)",
    )
    use_linter: bool = Field(
        default=True, description="Use ChkTeX linter for validation"
    )
    use_pdfcrop: bool = Field(default=False, description="Use pdfcrop to trim figures")
    model_name: Optional[str] = Field(
        default=None, description="LLM model to use (defaults to env DEFAULT_MODEL)"
    )
    base_url: Optional[str] = Field(
        default=None, description="Custom API base URL for OpenAI-compatible APIs"
    )
...
class PDFGenerateRequest(BaseModel):
    ...
    start_page: Optional[int] = Field(
        default=None, description="Starting page number (1-indexed, inclusive)"
    )
    end_page: Optional[int] = Field(
        default=None, description="Ending page number (1-indexed, inclusive)"
    )
...
class JobResponse(BaseModel):
    ...
    user_id: str = Field(
        ...,
        description="User ID for workspace isolation (reuse this for subsequent requests)",
    )
...
class EditSlidesRequest(BaseModel):
    ...
    use_paper_context: bool = Field(
        default=True, description="Include original paper context during editing"
    )
...
class EditSingleSlideRequest(BaseModel):
    ...
    slide_number: int = Field(
        ..., description="Slide/frame number to edit (1-indexed)", ge=1
    )
    instruction: str = Field(..., description="Editing instruction for the LLM")
...
class SpeakerNotesRequest(BaseModel):
    ...
    instruction: str = Field(
        default="",
        description="Optional custom instruction for speaker note generation",
    )
    api_key: Optional[str] = Field(default=None, description="API key (optional)")
    model_name: Optional[str] = Field(default=None, description="LLM model to use")
    base_url: Optional[str] = Field(default=None, description="Custom API base URL")
...
# Helper functions
def create_job(
    paper_id: str, source_type: str, user_id: Optional[str] = None
) -> tuple[str, str, str]:
...
def get_workspace_dir(user_id: str, paper_id: str) -> str:
    ...
    if job_key not in jobs:
        raise HTTPException(
            status_code=404, detail=f"Job ({user_id}, {paper_id}) not found"
        )
...
async def generate_slides_task(
    ...
    try:
        update_job(
            user_id, paper_id, JobStatus.GENERATING, "Generating slides from source..."
        )
        ...
        if source_type == "arxiv":
            ...
            logger.info(
                f"Generating slides for arXiv paper {paper_id} in workspace {workspace_dir}"
            )
            ...
            logger.info(
                f"Generating slides for PDF {paper_id} in workspace {workspace_dir}"
            )
            success = await asyncio.to_thread(
                generate_slides_from_pdf,
                pdf_path,
            ...
            logger.info(
                f"Generating slides for LaTeX zip {paper_id} in workspace {workspace_dir}"
            )
            success = await asyncio.to_thread(
                generate_slides_from_latex_zip,
                zip_path,
        ...
        if not success:
            ...
            if not os.path.exists(slides_path):
                raise RuntimeError(
                    f"Slide generation failed - slides.tex not created. Check LLM API logs."
                )
            else:
                raise RuntimeError(
                    "Slide generation failed - LLM returned unsuccessful status"
                )
        ...
        compile_success = await asyncio.to_thread(
            compile_latex, tex_filename, workspace_dir
        )
        ...
        update_job(
            user_id, paper_id, JobStatus.COMPLETED, "Slides generated successfully!"
        )
        logger.info(
            f"Job ({user_id}, {paper_id}) completed successfully in {workspace_dir}"
        )
        ...
        update_job(
            user_id, paper_id, JobStatus.FAILED, "Generation failed", error=error_msg
        )
    ...
    return {
        ...
        "active_jobs": len(
            [
                ...
                if j["status"]
                in [JobStatus.PENDING, JobStatus.GENERATING, JobStatus.COMPILING]
            ]
        ),
        "completed_jobs": len(
            [j for j in jobs.values() if j["status"] == JobStatus.COMPLETED]
        ),
    }
...
async def generate_from_arxiv(
    ...
    try:
        ...
        user_id, paper_id, workspace_dir = create_job(
            paper_id, "arxiv", request.user_id
        )
...
async def generate_from_pdf(
    ...
    start_page: Optional[int] = Query(
        default=None, description="Starting page (1-indexed)"
    ),
    end_page: Optional[int] = Query(
        default=None, description="Ending page (1-indexed)"
    ),
...
async def generate_from_latex_zip(
    ...
    try:
        ...
        assigned_user_id, paper_id, workspace_dir = create_job(
            paper_id, "latex_zip", user_id
        )
...
@app.get(
    "/status/{user_id}/{paper_id}", response_model=JobStatusResponse, tags=["Jobs"]
)
...
async def list_jobs(
    ...
    limit: int = Query(
        default=50, ge=1, le=1000, description="Maximum number of jobs to return"
    ),
):
    """
...
async def download_tex(user_id: str, paper_id: str):
    ...
    if not os.path.exists(tex_path):
        raise HTTPException(status_code=404, detail=f"TeX file not found at {tex_path}")
...
async def edit_slides_endpoint(
    ...
    try:
        ...
        # Recompile
        compile_success = await asyncio.to_thread(
            compile_latex, "slides.tex", workspace_dir
        )
...
async def edit_single_slide_endpoint(
    ...
    try:
        ...
        if not edited_code:
            raise HTTPException(
                status_code=500, detail=f"Failed to edit slide {slide_number}"
            )
...
async def generate_speaker_notes_endpoint(
    ...
    try:
        ...
        logger.error(
            f"Failed to generate speaker notes for job ({user_id}, {paper_id}): {e}"
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_speaker_notes_endpoint(user_id: str, paper_id: str):
    ...
    try:
        ...
        if not notes:
            raise HTTPException(
                status_code=404, detail="Speaker notes not found for this job"
            )
        ...
        logger.error(
            f"Failed to load speaker notes for job ({user_id}, {paper_id}): {e}"
        )
        raise HTTPException(status_code=500, detail=str(e))
